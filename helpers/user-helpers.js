  ...
  getOrderProducts: (orderId) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let orderItems = await connectdb
          .get()
          .collection(collection.ORDER_COLLECTION)
          .aggregate([
            {
            ...
            {
              $lookup: {
                // Lookup to fetch product details from the PRODUCT_COLLECTION
                from: collection.PRODUCT_COLLECTION,
                localField: "item",
                foreignField: "_id",
                as: "productDetails",
